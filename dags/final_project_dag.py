...
CLUSTER_ACCESS = 'sparkcluster-datapath-access'
CLUSTER_STAGE = 'sparkcluster-datapath-stage'
REGION = 'us-east1'
...
dag_args = {
    'dag_id': '8_final_project_etl_v6',
    'schedule': INTERVAL,
    'start_date': days_ago(1),
    'catchup': False,
    'description': 'Final project DAG - End to End',
    'default_args': {
        'owner': OWNER,
        'retries': 1,
    }
...
with DAG(**dag_args) as dag:
    ...
    with TaskGroup(group_id='quality_to_access_layer') as quality_to_access_layer:
        ...
        pyspark_job_qty_to_acess = {
            ...
            'pyspark_job': {
                ...
                'jar_file_uris': ['gs://spark-lib/bigquery/spark-bigquery-latest_2.12.jar'],
                # Python script that uses PySpark
                'main_python_file_uri': 'gs://spark-final-project-bucket/1_quality_to_access.py'
            }
        }
    ...
    with TaskGroup(group_id='access_to_stage_layer') as access_to_stage_layer:
        create_cluster_access = DataprocCreateClusterOperator(
            task_id='create_cluster_access',
            project_id=PROJECT_ID,
            cluster_name=CLUSTER_ACCESS,
            num_workers=2,
            # we need a bucket to store files from the cluster
            storage_bucket=SPARK_BUCKET,
            region=REGION,
        ...
        pyspark_job_access_to_stage = {
            'reference': {
                'project_id': PROJECT_ID,
                'job_id': f"access_to_stage_{uuid_run}"
            },
            'placement': {
                'cluster_name': CLUSTER_ACCESS
            },
            ...
            'pyspark_job': {
                ...
                'jar_file_uris': ['gs://spark-lib/bigquery/spark-bigquery-latest_2.12.jar'],
                # Python script that uses PySpark
                'main_python_file_uri': 'gs://spark-final-project-bucket/2_access_to_stage.py'
            }
        }

        tables_from_access_to_stage = DataprocSubmitJobOperator(
            task_id='tables_from_access_to_stage',
            project_id=PROJECT_ID,
            region=REGION,
            job=pyspark_job_access_to_stage,
            gcp_conn_id=GOOGLE_CONN
        )

        delete_cluster_access = DataprocDeleteClusterOperator(
            task_id='delete_cluster_access',
            project_id=PROJECT_ID,
            cluster_name=CLUSTER_ACCESS,
            region=REGION,
            trigger_rule='all_done'
        ...
        # order task group
        create_cluster_access >> tables_from_access_to_stage >> delete_cluster_access

    with TaskGroup(group_id='stage_to_bus_layer') as stage_to_bus_layer:
        create_cluster_stage = DataprocCreateClusterOperator(
            task_id='create_cluster_stage',
            project_id=PROJECT_ID,
            cluster_name=CLUSTER_STAGE,
            num_workers=2,
            # we need a bucket to store files from the cluster
            storage_bucket=SPARK_BUCKET,
            region=REGION,
        ...
        pyspark_job_stage_to_business = {
            'reference': {
                'project_id': PROJECT_ID,
                'job_id': f"stage_to_bus_{uuid_run}"
            },
            'placement': {
                'cluster_name': CLUSTER_STAGE
            },
            ...
            'pyspark_job': {
                ...
                'jar_file_uris': ['gs://spark-lib/bigquery/spark-bigquery-latest_2.12.jar'],
                # Python script that uses PySpark
                'main_python_file_uri': 'gs://spark-final-project-bucket/3_stage_to_business.py'
            }
        }

        tables_from_stage_to_business = DataprocSubmitJobOperator(
            task_id='tables_from_stage_to_business',
            project_id=PROJECT_ID,
            region=REGION,
            job=pyspark_job_stage_to_business,
            gcp_conn_id=GOOGLE_CONN
        )

        delete_cluster_stage = DataprocDeleteClusterOperator(
            task_id='delete_cluster_stage',
            project_id=PROJECT_ID,
            cluster_name=CLUSTER_STAGE,
            region=REGION,
            trigger_rule='all_done'
        ...
        # order task group
        create_cluster_stage >> tables_from_stage_to_business >> delete_cluster_stage
